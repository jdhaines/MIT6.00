"""Prewritten test function for doing unit testing of ps3a.py."""

from ps3a import (deal_hand, HAND_SIZE, get_word_score, update_hand,
                  get_frequency_dict, is_valid_word, load_words)
...
def test_deal_hand():
    ...
    for k in hand.keys():
        if (not type(k) is str) or (not type(hand[k]) is int):
            print("FAILURE: test_deal_hand()")
            print("\tUnexpected type of dictionary: string -> int expected, ",
                  "but was", type(k), "->", type(hand[k]))
    ...
    if num != HAND_SIZE:
            ...
            print("\tAsked for a hand of size", HAND_SIZE, "but it returned",
                  "a hand of size", num)
    ...
    if repeats > 10:
        print("FAILURE: test_deal_hand()")
        print("\tSame hand returned", repeats, "times by deal_hand(). ",
...
def test_update_hand():
    ...
    if hand2 != expected_hand1 and hand2 != expected_hand2:
        ...
        print("\tReturned: ", hand2, "-- but expected:", expected_hand1,
              "or", expected_hand2)
    ...
    if hand2 != expected_hand1 and hand2 != expected_hand2:
        print("FAILURE: test_update_hand('" + word + "', " + str(hand) + ")")
        print("\tReturned: ", hand2, "-- but expected:",
